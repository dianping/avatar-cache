...
public class EhcacheClientImpl implements CacheClient, Lifecycle, InitialConfiguration {
	...
	public void add(String key, Object value, int expiration, String category) {
		if (!findCache(category).isKeyInCache(key)) {
			findCache(category).put(new Element(key, value, Boolean.FALSE, Integer.valueOf(0), Integer.valueOf(expiration)));
		}
	}
	...
	public long decrement(String key, int amount, String category) {
		...
		if (element != null) {
			...
			if (value instanceof Long) {
				...
				findCache(category).put(new Element(key, newValue, Boolean.FALSE, Integer.valueOf(0), Integer.valueOf(element
						.getTimeToLive())));
				return newValue;
			}
		}
	...
	public <T> Map<String, T> getBulk(Collection<String> keys, Map<String, String> categories) {
		...
		for (String key : keys) {
			map.put(key, (T) findCache(categories == null ? null : categories.get(key)).get(key).getObjectValue());
		}
		return map;
	}
	...
	public void set(String key, Object value, int expiration, boolean isHot, String category) {
		findCache(category).put(new Element(key, value, Boolean.FALSE, Integer.valueOf(0), Integer.valueOf(expiration)));
		if (isHot) {
			findCache(category).put(new Element(key + "_bak", value, Boolean.TRUE, Integer.valueOf(0), Integer.valueOf(0)));
			...
			if (result == null) {
				...
				try {
					...
					if (findCache(category).get(key + "_lock") == null) {
						findCache(category).put(new Element(key + "_lock", true, Boolean.FALSE, Integer.valueOf(0), Integer
